---
name: a-stock-tdx-adapter
description: "通达信股票数据分析技能 - 提供股票信息查询、公司F10资料获取、技术指标分析和信号筛选功能"
license: MIT
metadata:
...
```bash
python scripts/src/main.py info <股票代码> [--days <日K天数>] [--weeks <周K周数>] [--months <月K月数>]
```
...
```bash
# 查询贵州茅台的详细信息（默认参数）
python scripts/src/main.py info 600519

# 查询平安银行，指定日K20天、周K10周、月K6个月
python scripts/src/main.py info 000001 --days 20 --weeks 10 --months 6

# 使用短参数名
python scripts/src/main.py info 000001 -d 20 -w 10 -m 6
```
...
```bash
python scripts/src/main.py company <股票代码> [--blocks <块名1> <块名2> ...]
```
...
```bash
# 获取贵州茅台的全部公司信息
python scripts/src/main.py company 600519

# 只获取公司概况和财务分析
python scripts/src/main.py company 600519 --blocks "公司概况" "财务分析"

# 使用短参数名
python scripts/src/main.py company 600519 -b "公司概况" "财务分析"
```
...
```bash
python scripts/src/main.py screen <股票代码列表> --signal <信号类型> [--ktype <K线类型>] [--startday <开始日期>] [--endday <结束日期>]
```
...
```bash
# 筛选出现金叉信号的股票（日K）
python scripts/src/main.py screen "600519,000001,000858" --signal golden_cross

# 筛选超卖股票（周K）
python scripts/src/main.py screen "600519,000001" --ktype week

# 指定日期范围筛选
python scripts/src/main.py screen "600519,000001" --signal death_cross --startday 20260101 --endday 20260708

# 使用短参数名
python scripts/src/main.py screen "600519,000001" -s golden_cross -k day -t 20260101 -e 20260708
```
...
```bash
python scripts/src/main.py signals
```
...
# 2. 筛选金叉股票
python scripts/src/main.py screen "600519,000001,000858,601318" --signal golden_cross
...
```bash
# 获取多周期技术指标
python scripts/src/main.py info 600519 --days 20 --weeks 10 --months 6
```
